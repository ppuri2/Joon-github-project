...
quick-rules와 동일한 --check 게이트 + 전 패턴 전수 커버 + 빈 항목 0 +
부피 상한을 여기서 고정한다.
...
class DiagnosisRulesBuildTests(unittest.TestCase):
    ...
    def test_all_taxonomy_ids_covered(self) -> None:
        ...
        # 개수를 하드코딩하면 패턴을 추가할 때마다 테스트가 깨진다(v2.5에서 D-8·D-9 신설).
        # 요건은 "taxonomy의 모든 ID가 렌더에 있다"이지 개수 자체가 아니다.
    ...
    def test_no_empty_definitions_or_signatures(self) -> None:
        """패턴당 시그니처 줄이 taxonomy 패턴 수와 일치, 빈 값 0."""
